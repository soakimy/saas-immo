...
create table leads (
  ...
  ai_recommended_actions jsonb,          -- liste d'actions suggérées par l'IA, ex: [{"label": "Appeler aujourd'hui", "urgent": true}]
  ai_score_factors jsonb,                -- facteurs ayant influencé le score, ex: [{"label": "Budget compatible", "impact": "positive", "detail": "..."}]
  ai_last_qualified_at timestamptz,      -- date de la dernière qualification IA
  budget_min numeric,
  budget_max numeric,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
