...
export default function MatchingPanel({
  ...
  // Déclenché une seule fois si autoLoad est vrai (uniquement juste après
  // la création d'un bien/contact, pas à chaque visite de la fiche, pour
  // éviter d'appeler l'IA inutilement).
  useEffect(() => {
    if (autoLoad) {
      handleLoad();
    }
  ...
  return (
    <div>
      ...
      {matches.length === 0 ? (
        <p className="text-sm text-ink/40 italic mb-2">{emptyLabel}</p>
      ) : (
      ...
      <div className="flex items-center justify-between">
        ...
        <p className="text-xs text-ink/40">
          Estimation indicative de l&apos;IA, pas un calcul garanti.
        </p>
      </div>
    </div>
  );
