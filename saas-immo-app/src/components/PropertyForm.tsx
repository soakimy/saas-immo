...
export default function PropertyForm({
  ...
  const [form, setForm] = useState({
    type: existingProperty?.type ?? "appartement",
    transaction_type: existingProperty?.transaction_type ?? "vente",
    title: existingProperty?.title ?? "",
    price: existingProperty?.price?.toString() ?? "",
    surface_m2: existingProperty?.surface_m2?.toString() ?? "",
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { data: insertedOrUpdated, error } = isEditing
      ? await supabase.from("properties").update(payload).eq("id", existingProperty.id).select("id").single()
      : await supabase.from("properties").insert({ ...payload, agency_id: agencyId }).select("id").single();
    ...
    const propertyId = insertedOrUpdated?.id ?? existingProperty?.id;
    const destination = isEditing
      ? `/dashboard/biens/${propertyId}`
      : `/dashboard/biens/${propertyId}?justCreated=true`;
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white border border-line rounded-lg p-8 space-y-5">
      ...
      >
        {loading ? "Enregistrement..." : isEditing ? "Enregistrer les modifications" : "Ajouter le bien"}
      </button>
    </form>
  );
}
