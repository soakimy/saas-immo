...
export default async function ContactDetailPage({
  ...
  const { data: leads } = await supabase
    .from("leads")
    .select("id, status, ai_score, ai_summary, ai_recommended_actions, ai_score_factors, budget_min, budget_max, properties(id, title, type, city, price)")
    ...
    <main className="min-h-screen bg-paper">
      ...
      <section className="max-w-2xl mx-auto px-6 py-10">
        ...
        <div className="bg-white border border-line rounded-lg p-8 mb-6">
            ...
            <div className="mt-4 pt-4 border-t border-line">
              ...
              {aiResult ? (
                <div>
                  ...
                  {aiResult.ai_recommended_actions && aiResult.ai_recommended_actions.length > 0 && (
                    <div className="flex flex-wrap gap-2 mb-3">
                      {aiResult.ai_recommended_actions.map((action: string, i: number) => (
                        <span
                          key={i}
                          className="text-xs font-medium px-2.5 py-1 rounded-full bg-ink/5 text-ink border border-line"
                        >
                          ✅ {action}
                        </span>
                      ))}
                  ...
                  <ScoreExplanation factors={aiResult.ai_score_factors} />
                </div>
              ) : (
          ...
          <div className="mt-4 pt-4 border-t border-line">
            <p className="text-ink/50 text-sm mb-3">Biens compatibles</p>
            <MatchingPanel
              apiUrl="/api/contacts/match-properties"
              bodyKey="contactId"
              bodyValue={contact.id}
              linkPrefix="/dashboard/biens"
              emptyLabel="Aucun bien compatible identifié pour l'instant."
              autoLoad={searchParams.justCreated === "true"}
            />
          </div>
        ...
        <h2 className="font-display text-lg text-ink mb-3 mt-10">Historique</h2>
...
function ScoreBadge({ score }: { score: number }) {
  ...
  return (
    <span className={`text-sm font-medium px-2.5 py-1 rounded-full whitespace-nowrap ${color}`}>
      {label}
    </span>
  );
}
