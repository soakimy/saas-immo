...
export default async function PropertiesPage() {
  ...
  const { data: properties } = await supabase
    .from("properties")
    .select("id, title, type, transaction_type, status, price, city, surface_m2")
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {properties.map((property) => (
              <Link
                ...
                className="block border border-line rounded-lg bg-white p-6 hover:border-clay/50 transition-colors"
              >
