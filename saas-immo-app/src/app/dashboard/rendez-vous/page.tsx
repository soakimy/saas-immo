...
import VisitReportButton from "@/components/VisitReportButton";
import CloseAppointmentButton from "@/components/CloseAppointmentButton";
...
function AppointmentRow({ appt }: { appt: any }) {
  ...
  return (
    <div className="relative flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-6 py-4">
      ...
      <div className="flex items-center gap-4 pl-20 sm:pl-0">
        <StatusBadge status={appt.status} scheduledAt={appt.scheduled_at} />
        {needsClosing(appt.status, appt.scheduled_at) && (
          <CloseAppointmentButton appointmentId={appt.id} />
        )}
        {appt.type === "visit" && <VisitReportButton appointmentId={appt.id} />}
        <DeleteButton
          table="appointments"
          id={appt.id}
...
function needsClosing(status: string, scheduledAt: string): boolean {
  const isPast = new Date(scheduledAt) < new Date();
  const stillPending = status === "scheduled" || status === "confirmed";
  return isPast && stillPending;
}

function StatusBadge({ status, scheduledAt }: { status: string; scheduledAt: string }) {
  if (needsClosing(status, scheduledAt)) {
    return (
