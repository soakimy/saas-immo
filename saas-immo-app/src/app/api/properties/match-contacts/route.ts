...
export async function POST(request: NextRequest) {
  ...
  const contactsDescription = contacts
    .map((c: any, i: number) => {
      ...
      return `Contact ${i + 1} : ${c.first_name ?? ""} ${c.last_name ?? ""}, type ${c.type}. Budget : ${lead?.budget_min ?? "non précisé"} - ${lead?.budget_max ?? "non précisé"} €. Notes : ${c.notes ?? "aucune"}.`;
    })
    .join("\n");
...
{"matches": [{"contact_number": le numéro du contact tel qu'indiqué ci-dessus (entier), "score": un entier entre 0 et 100, "reason": "raison courte en 8 mots maximum"}]}

Règles importantes :
- Utilise UNIQUEMENT le numéro du contact (1, 2, 3...), jamais son nom ou un identifiant.
...
- Trie par score décroissant.`;
  ...
  let parsed: { matches: { contact_number: number; score: number; reason: string }[] };
  try {
  ...
  const enrichedMatches = parsed.matches
    .map((m) => {
      const contact = contacts[m.contact_number - 1] as any;
