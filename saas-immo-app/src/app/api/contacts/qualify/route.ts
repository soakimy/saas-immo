...
Réponds STRICTEMENT au format JSON suivant, sans aucun texte avant ou après, sans balises markdown :
{
  ...
  "factors": une liste de 3 à 5 facteurs ayant influencé le score, chacun au format {"label": "nom court du facteur (2-4 mots)", "impact": "positive" | "negative" | "neutral", "detail": "explication en moins de 12 mots"}. Exemples de facteurs possibles : compatibilité budget/prix, présence de notes détaillées, financement mentionné ou non, clarté de la recherche exprimée, ancienneté du contact. N'invente pas un facteur s'il n'y a aucune information pour le juger : utilise alors impact "neutral" avec un detail du type "non précisé".
}

Base le score, les actions et les facteurs uniquement sur les informations données ci-dessus. Si peu d'informations sont disponibles, donne un score prudent autour de 40-50 et des facteurs qui reflètent honnêtement ce manque d'information (impact "neutral"), plutôt que d'inventer un niveau d'urgence ou de certitude qui n'est pas justifié par les données.`;
  ...
  // On met à jour tous les leads de ce contact avec le même score/résumé/actions/facteurs
  const { error: updateError } = await supabase
    ...
    .update({
      ...
      ai_score_factors: parsed.factors,
