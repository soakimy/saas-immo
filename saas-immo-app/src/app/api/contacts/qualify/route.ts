...
{"score": un nombre entre 0 et 100 représentant la probabilité que ce contact aboutisse à une transaction, "summary": un résumé en 1-2 phrases de la situation et de ce qu'il faudrait faire ensuite}

Base ton score uniquement sur les informations données ci-dessus (présence de notes détaillées, cohérence budget/bien, source du contact). Si peu d'informations sont disponibles, donne un score prudent autour de 40-50, pas un score extrême.`;
  ...
  // On met à jour tous les leads de ce contact avec le même score/résumé
  const { error: updateError } = await supabase
    .from("leads")
    .update({ ai_score: parsed.score, ai_summary: parsed.summary })
    .eq("contact_id", contactId);
