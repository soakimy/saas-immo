...
export async function POST(request: NextRequest) {
  ...
  const propertiesDescription = properties
    .map(
      (p, i) =>
        `Bien ${i + 1} : ${p.type} en ${p.transaction_type} à ${p.city ?? "ville non précisée"}, ${p.price ?? "prix non précisé"} €, ${p.surface_m2 ?? "?"} m², ${p.rooms ?? "?"} pièces.`
    )
    .join("\n");
...
Biens disponibles (numérotés) :
${propertiesDescription}

Réponds STRICTEMENT en JSON, sans aucun texte avant/après, sans balises markdown :
{"matches": [{"property_number": le numéro du bien tel qu'indiqué ci-dessus (entier), "score": un entier entre 0 et 100, "reason": "raison courte en 8 mots maximum"}]}

Règles importantes :
- Utilise UNIQUEMENT le numéro du bien (1, 2, 3...), jamais son nom ou un identifiant.
...
- Trie par score décroissant.`;
  ...
  let parsed: { matches: { property_number: number; score: number; reason: string }[] };
  try {
  ...
  const enrichedMatches = parsed.matches
    .map((m) => {
      const property = properties[m.property_number - 1];
